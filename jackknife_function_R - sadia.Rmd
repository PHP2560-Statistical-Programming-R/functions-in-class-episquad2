...
jackknife.lm <- function(df,formula,p) {
  ...
  for (i in 1:nrow(df)){
   coeff[,i]<-lm(formula, df[-i,])$coefficients}
  
   var.coeff<-c()
   for(i in 1:nrow(coeff)){
   var.coeff[i]<-var(coeff[,i], na.rm=TRUE)}
   
   jackknife.stderr<-vector("double", length=p)
   for(i in 1:length(var.coeff)){
   jackknife.stderr[i]<-((n-1)^2/n*var.coeff[i])^(1/2)}
